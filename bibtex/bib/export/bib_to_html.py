...
def layout_begin_for_mode(begin: str, mode: str) -> str:
    begin = normalize_javascript(begin).replace("\\encoding", "UTF-8")
    if mode == "full":
        return begin

    script_start = begin.index('<script type="text/javascript">')
    begin = begin[script_start:]
...
def render_document(entries: list[dict[str, str]], mode: str, order: str) -> str:
    ...
    begin = layout_begin_for_mode(begin_path.read_text(encoding="utf-8"), mode)
